...
public class RabbitMqConnectionManager : IAsyncDisposable
{
    ...
    /// <summary>
    /// 获取或创建 RabbitMQ 连接和通道，确保 Exchange 已声明
    /// </summary>
    public async Task<IModel> GetChannelAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "RabbitMQ connected to {Host}:{Port}{VHost}, exchange: {Exchange}",
                _config.HostName, _config.Port, _config.VirtualHost, _config.Exchange);
