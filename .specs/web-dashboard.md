...
按"可独立验证的功能模块"拆，建议 7 个任务，每个 1-2 天工作量：

#	任务	交付物	依赖
1	项目脚手架 + 布局骨架	Vite 项目 + 路由 + 侧边栏 + 工厂选择器 + 响应式布局	无
2	API 层 + Pinia Stores	axios 封装 + 4 个 store + TypeScript 接口	#1
3	SignalR 实时推送	composable + 连接管理 + 重连 + 分组切换 + 声音告警	#2
4	报警看板	统计卡片 + 实时列表 + 通知弹窗 + unreadCount	#2 #3
5	报警管理 + 详情	报警表格 + 过滤分页 + 详情页 + 确认/抑制/关闭操作	#2
6	规则管理	规则表格 + 新建/编辑表单 + 启停 + 删除 + 校验	#2
7	设备状态 + Docker 部署	饼图 + 趋势图 + 离线列表 + nginx + Dockerfile 联调	#2 #3
关键路径：#1 → #2 → #3 → #4，大约 5-6 天打通核心流程。#5 #6 可以和 #4 并行，#7 收尾。

核心原则：每个任务结束都能跑能看——#1 结束能看到空壳页面，#2 结束能调通 API，#3 结束能收到实时推送，以此类推。
