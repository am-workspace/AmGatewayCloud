...
| 优先级 | 条目 | 状态 | 理由 |
|-------|------|------|------|
| 🔴 P0 | Count 上限校验（1.1） | ✅ 已实现 | 运行时协议异常，难以排查 |
| 🔴 P0 | DataPoint.Value 序列化（4.1） | ✅ 已实现 | 阶段3 RabbitMQ 必需，延后是大坑 |
| 🔴 P0 | 全部失败主动重连（3.1） | ✅ 已实现 | 断线后空转，数据丢失 |
| 🔴 P0 | CancellationToken 传入 Delay（3.3） | ✅ 已实现 | 不传则关机卡住 |
| 🟡 P1 | 连接超时配置（2.1） | ✅ 已实现 | 首次连接可能卡 20s+ |
| 🟡 P1 | 指数退避（2.2） | ✅ 已实现 | 防止重连风暴刷屏日志 |
| 🟡 P1 | 地址区间重叠检测（1.2） | ✅ 已实现 | 配置错误时数据混乱 |
| 🟡 P1 | Tags 非空校验（1.3） | ✅ 已实现 | 配置缺失时静默出错 |
| 🟡 P1 | volatile IsConnected（2.3） | ✅ 已实现 | 多线程可见性 |
| 🟢 P2 | Scale + Offset（5.1） | ✅ 已实现 | 功能增强，可后续迭代 |
| 🟢 P2 | 采集质量标记（4.2） | ✅ 已实现 | 工业标准，可后续迭代 |
| 🟢 P2 | 严格轮询周期（3.2） | ❌ 未做 | 看场景需求，当前 Task.Delay 够用 |
| ⚪ P3 | 多设备支持（9.1） | ❌ 未做 | 阶段1单设备够用，后续按需 |
| ⚪ P3 | NModbus 版本锁定（9.2） | ✅ 已锁定 3.0.83 | 与原 AmGateway 一致 |
