...
| **阶段3a** | 新增 `RabbitMqOutput : IDataOutput`；抽取 `Collector.Abstractions`（DataPoint + IDataOutput + DataQuality + MapValueType） | RabbitMQ 部署；**阶段3前必须完成 Abstractions 抽取** |
| **阶段3b** | 无变化（采集器本身不变） | — |
| **阶段4** | 无变化 | — |
...
├── Program.cs                              # 入口 + DI + 异常处理
├── appsettings.json                        # 4 组 40 节点默认配置
├── OpcUaSession.cs                         # 会话管理 + 自动重连 + 命名空间解析
├── OpcUaCollectorService.cs                # BackgroundService 采集主服务
├── Configuration/
│   ├── CollectorConfig.cs                  # 配置映射类（含 Writable/DeadbandPercent 伏笔）
...
collector-opcua-status.md    ← 本文件（实现状态/遗留项/测试/演进）
```
